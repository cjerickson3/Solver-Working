...
def distance_signature(relative_angles):
    ...
    """
    if len(relative_angles.shape) == 2:
        relative_angles = relative_angles.flatten()
        print("distance_signature - relative_angle was 2D")
    length = relative_angles.shape[0]
    
    l1 = np.array([0, float(relative_angles[0])])
    l2 = np.array([length - 1, float(relative_angles[-1])])
...
def flat_score(relative_angles):
    """
        ...
        :return: List of floats
    """
    print("In flat_score:", relative_angles.shape)
    length = relative_angles.shape[0]
...
def indent_score(relative_angles):
    """
        ...
        :return: List of floats
    """
    print("In indent_score:", relative_angles.shape)
    length = relative_angles.shape[0]
...
def compute_comp(combs_l, relative_angles, method='correlate'):
    ...
    for comb_t in combs_l:
        ...
        for comb in comb_t:
            ...
            relative_angles_flat = np.array(relative_angles_tmp[comb[0]:comb[1]]).reshape(-1)
            if method == 'flat':
                hole = indent_score(relative_angles_flat)
                head = outdent_score(relative_angles_flat)
                border = flat_score(relative_angles_flat)
        
            if hole != border:
                results_comp.append(np.min([hole, head]))
            else:
                results_comp.append(border[0])
...
def my_find_corner_signature(cnt, green=False):
    ...
    while sigma <= max_sigma:
        ...
        print("Shape after get_relative_angles", relative_angles.shape)
        relative_angles = np.array(relative_angles)
        print("Shape after np.array:", relative_angles.shape)
        ...
        print("Shape after np.roll-1", relative_angles.shape)
        ...
        relative_angles = np.roll(relative_angles, -int(len(relative_angles) / 2))
        print("Shape after np.roll-2", relative_angles.shape)
        ...
        print("Shape after sklearn", relative_angles.shape)
        ...
        print("my_find_corner_signature:", relative_angles.shape)
        best_fit += offset
