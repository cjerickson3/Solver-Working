...
class Extractor():
    ...
    def extract(self):
        ...
        for i, contour in enumerate(contours):
            print(f"Contour {i}: shape = {contour.shape}, content = {contour}")
        ...
        if len(sys.argv) < 0:
            ...
            #  Fix inhomogeneous error
            # contours = sorted(np.array(contours), key=lambda x: x.shape[0], reverse=True)
            contours = sorted(contours, key=lambda x: x.shape[0], reverse=True)[:nb_pieces]
            contours = np.array(sorted(contours, key=lambda x: x.shape[0], reverse=True)[:nb_pieces], dtype=object)
          
            max = contours[1].shape[0]
            # contours = np.array([elt for elt in contours if elt.shape[0] > max / 
            contours = np.array([elt for elt in contours if elt.shape[0] > max / 3], dtype=object)
